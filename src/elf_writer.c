 ...
 *
 * Strategy:
 *   1. Parse the GNU AS text produced by backend_arm64/x86_64 into
 *      a simple section/symbol/reloc model.
 *   2. Encode x86_64 instructions to binary with a minimal table-
 *      driven encoder covering the exact instruction set emitted by b1cc.
 *   3. Lay out ELF64 (x86_64) sections and write bytes.
 *
...
static int x64_encode_line(const char *line, ByteBuf *text, EncCtx *ctx,
    ...
    /* ---- je / jne / jmp (local labels only in b1cc output) ---- */
    if (strcmp(mnem, "je") == 0 || strcmp(mnem, "jne") == 0 || strcmp(mnem, "jmp") == 0) {
        char label[64]; int nl = 0;
        while (*p && *p != '\n' && *p != ' ' && nl < 63) label[nl++] = *p++;
        label[nl] = 0;
        int is_cond = (strcmp(mnem, "je") == 0 || strcmp(mnem, "jne") == 0);
        uint8_t opcode = is_cond ? 0x0F : 0;
        uint8_t opcode2 = (strcmp(mnem, "je") == 0) ? 0x84 : ((strcmp(mnem, "jne") == 0) ? 0x85 : 0xE9);

        int32_t target_off = ctx_find_label(ctx, label);
        if (target_off >= 0) {
            /* Known label: backward jump */
            if (is_cond) bb_write8(text, 0x0F);
...
static void parse_asm_x64(const char *asm_text, const char *src_path, AsmModel *m) {
    ...
    char pending_globl2[256] = "";
    char pending_weak[256] = "";
    ...
    while (*p) {
        ...
        /* Section switches */
        if (strcmp(lp, ".text") == 0 || strncmp(lp, ".section .text.", 15) == 0) { cur_sec = SEC_TEXT; in_function = 0; continue; }
        if (strcmp(lp, ".data") == 0) { cur_sec = SEC_DATA; in_function = 0; continue; }
        if (strcmp(lp, ".section .rodata") == 0 ||
            strcmp(lp, ".rodata") == 0 ||
            strncmp(lp, ".section\t.rodata", 16) == 0 ||
            strncmp(lp, ".section .rodata", 16) == 0) {
            cur_sec = SEC_RODATA; in_function = 0; continue;
        }
        if (strncmp(lp, ".section .data.", 14) == 0) { cur_sec = SEC_DATA; in_function = 0; continue; }
        if (strncmp(lp, ".section .rodata.", 16) == 0) { cur_sec = SEC_RODATA; in_function = 0; continue; }
        if (strncmp(lp, ".section .bss.", 13) == 0) { cur_sec = SEC_BSS; in_function = 0; continue; }
        if (strcmp(lp, ".bss") == 0) { cur_sec = SEC_BSS; in_function = 0; continue; }

        /* .globl declaration */
        if (strncmp(lp, ".globl ", 7) == 0 || strncmp(lp, ".global ", 8) == 0) {
            const char *s = lp + (lp[6] == ' ' ? 7 : 8);
            s = skip_ws(s);
            if (pending_globl[0]) strncpy(pending_globl2, pending_globl, 255);
            strncpy(pending_globl, s, 255);
            continue;
        }

        /* Weak binding declaration applies to the following symbol label. */
        if (strncmp(lp, ".weak ", 6) == 0) {
            const char *s = skip_ws(lp + 6);
            strncpy(pending_weak, s, 255);
            continue;
        }
        ...
        if (*lp == '.') {
            ...
            if (*q == ':') {
                if (in_function && cur_sec == SEC_TEXT) {
                    ...
                    sym.size    = 0;
                    sym.bind    = (strcmp(lname, pending_weak) == 0) ? STB_WEAK :
                                  ((strcmp(lname, pending_globl) == 0 || strcmp(lname, pending_globl2) == 0) ? STB_GLOBAL : STB_LOCAL);
                    sym.type    = STT_OBJECT;
                    sym.defined = 1;
        ...
        /* Label definition (ends with ':') */
        {
            ...
            if (*q == ':') {
                ...
                if (strcmp(lname, pending_type_func) == 0) {
                    ...
                    if (strcmp(lname, pending_globl) == 0) pending_globl[0] = 0;
                    if (strcmp(lname, pending_globl2) == 0) pending_globl2[0] = 0;
                } else {
                    if (!in_function && cur_sec != SEC_DATA && cur_sec != SEC_RODATA &&
                        strcmp(lname, pending_globl) == 0) {
                        ElfSym sym = {0};
                        strncpy(sym.name, lname, 255);
                        sym.shndx = 0;
                        sym.value = (uint64_t)m->text.size;
                        sym.size = 0;
                        sym.bind = (strcmp(lname, pending_weak) == 0) ? STB_WEAK : STB_GLOBAL;
                        sym.type = STT_NOTYPE;
                        sym.defined = 1;
                        symarr_push(&m->syms, &sym);
                        pending_globl[0] = 0;
                    ...
                    if (in_function) {
                        ...
                        sym.value   = (uint64_t)sec_buf->size;
                        sym.size    = 0;
                        sym.bind    = (strcmp(lname, pending_globl) == 0 || strcmp(lname, pending_globl2) == 0) ? STB_GLOBAL : STB_LOCAL;
                        sym.type    = STT_OBJECT;
                        sym.defined = 1;
